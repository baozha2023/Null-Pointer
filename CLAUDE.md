...
- Fixed floor layout: floor 0 = start, floors 1-3 = easy combats, floor 4 = shop, floor 5 = chest, floor 6 = rest, floor 7 = elite, floors 8-10 = hard combats, last floor = Boss
- Configurable `floors_per_act` (default 10), `locations_per_floor` (default 5)
- 50% obfuscation rate (locations show "?"), 30% chance to replace combats with non-combat events
- Branching connections: each node connects to the node directly above, above-left, and above-right (Slay the Spire-style)
- Generated via `rng_world_generation` track
