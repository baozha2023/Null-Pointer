...
| 5 | `ProfileStore` | SQLite-backed profile progression, achievement state, and run history |
...
- `[card_name:card_id]` — dynamically replaced with the card's name, colored according to its color_id.
...
CardPackData, ArtifactPackData, and ConsumablePackData define filtered subsets of content. Content packs auto-generate filter caches at startup (`Global._generate_card_pack_cache()` etc.). Adding a card to a pack automatically makes it available in relevant drafts and reward pools. Shops deliberately generate six character-color cards plus two white cards from separate filtered rarity pools.
...
Achievements are definitions, not per-achievement scripts. `AchievementData` contains presentation, one or more OR-connected triggers, AND-connected conditions, an optional progress aggregator, a run policy, and runtime-only provenance. Native definitions are generated by `GlobalProdDataGenerator`; mods load the same schema from `achievements/` JSON. Invalid or duplicate definitions are isolated without replacing an earlier ID, and external data can never mark itself as native.

`AchievementManager` indexes definitions by stable event ID and bridges gameplay signals into typed event dictionaries. Built-in events are `enemy_killed`, `combat_stat_changed`, `turn_completed`, `combat_completed`, `run_completed`, `location_entered`, `achievement_unlocked`, and `custom_signal:<id>`. Mods may also call `submit_event()` directly. Custom evaluators are optional and must inherit `BaseAchievementEvaluator`; they return evaluation data and never write persistence directly.

Progress supports `COUNT`, `SUM`, `LATEST`, `MAXIMUM`, `MINIMUM`, and `UNIQUE_COUNT` across `LIFETIME`, `RUN`, `COMBAT`, and `TURN` scopes. `ProfileStore` atomically persists current/latest/best values, update counts, scope keys, recent samples, unique values, and unlock timestamps. Scope identity is saved in `PlayerData`, so loading an active run neither resets nor leaks progress into a different run. Native achievements are standard-run-only and platform-synced; mod achievements remain local. See `ACHIEVEMENT_MODDING.md` for the JSON contract.
