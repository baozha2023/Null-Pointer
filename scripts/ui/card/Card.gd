...
func get_card_description(selected_target: BaseCombatant = null) -> String:
	...
	# TextParser handles non-intercepted variables, status icons, and default energy icons
	modified_description_bb_code = TextParser.parse(modified_description_bb_code, card_data.card_values)

	# Replace the default energy icons generated by TextParser with character-specific colored icons
	if card_data.card_color_id != "":
		var color_data: ColorData = Global.get_color_data(card_data.card_color_id)
		if color_data != null and color_data.color_energy_icon_texture_path != "":
			var image_bb_code: String = "[img width={0}]{1}[/img]".format([CARD_TEXT_IMAGE_SIZE, color_data.color_energy_icon_texture_path])
			modified_description_bb_code = modified_description_bb_code.replace(TextParser.ENERGY_ICON_BBCODE, image_bb_code)
