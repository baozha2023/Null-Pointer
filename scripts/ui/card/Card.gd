...
@onready var card_background: TextureRect = %Background
...
@onready var card_energy_sprite: ColorRect = %EnergySprite
...
func init(_card_data: CardData, angular_offset: float, connect_combat_signals: bool = false, connect_ui_signals: bool = true, is_in_codex: bool = false):
...
func update_card_display(selected_enemy: Enemy = null) -> void:
	...
	# update visuals
	if _is_in_codex and not card_data.is_discovered():
		card_texture.texture = FileLoader.load_texture("sprites/card/card_locked.png")
	else:
...
func get_card_description(selected_target: BaseCombatant = null) -> String:
			...
			if card_data.card_description.contains("[" + key_name + "]") or card_data.card_description.contains("[" + key_name + "_energy_icons]"):
				...
				if final_val <= 0:
					...
					for i in range(final_val):
						icons_str += TextParser.ENERGY_ICON_KEYWORD
	...
	if modified_description_bb_code.contains("[variable_energy_icons]"):
		...
		var variable_icons_str: String = "X" + offset_str + " " + TextParser.ENERGY_ICON_KEYWORD
		if Global.player_data != null and card_data.card_energy_cost_is_variable:
			...
			if consumed_energy <= 0:
				...
				for i in range(consumed_energy):
					variable_icons_str += TextParser.ENERGY_ICON_KEYWORD
		modified_description_bb_code = modified_description_bb_code.replace("[variable_energy_icons]", variable_icons_str)
	
	# TextParser handles non-intercepted variables, status icons, and energy icons
	var current_font_size: int = card_description.get("_current_font_size") if card_description.get("_current_font_size") != null else 20
	modified_description_bb_code = TextParser.parse(modified_description_bb_code, card_data.card_values, current_font_size)
